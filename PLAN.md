...
## Циклы 99–104 — хартбит (#19, сделано)

CORE-сервис (БЕЗ LLM-стека) над client+storage (паттерн `watch.py`/`moderation.py`):
пинги по расписанию с шаблонами и предохранителями. Зависит от storage (#13).

- **99 — `schedule=` в core**: `client.send_text(peer, text, reply_to=None, schedule=
  timedelta|datetime|None)` прокидывает `schedule` в Telethon `send_message` (серверная
  отложенная отправка); без него — прежнее поведение (регресс-тест). Инвалидация history
  как обычно. Fake `send_message` в conftest расширен `schedule=`.
- **100 — планы в storage (`core/heartbeat.py`)**: таблицы `heartbeat_plans`
  (PK `peer`, `templates` JSON, `interval_hours`, `jitter_minutes`, `quiet_start/end`,
  `enabled`, `last_ping_at`, `max_per_day`, `pings_today`, `day_start`) + `heartbeat_log`;
  `register_heartbeat_migrations(storage)`; Pydantic `HeartbeatPlan` (templates non-empty);
  CRUD `add_plan`(upsert)/`list_plans`/`list_enabled_plans`/`remove_plan`. Тесты на tmp
  (БЕЗ importorskip — это core+storage), roundtrip.
- **101 — расчёт «пора» (`is_due`, чистая функция, `rng`/`now` инжектятся)**: учитывает
  `interval_hours` + `jitter_minutes` (rng фиксирован в тесте → детерминизм), quiet hours
  (локальный час из `datetime.fromtimestamp(now)`; окно через полночь), `max_per_day`
  (исчерпан в текущем дне → не пора; сброс по `day_start` старше 24ч). Parametrized без sleep.
- **102 — предохранитель «наше без ответа»**: перед пингом `client.history(peer, limit=1)`;
  если хвост `out=True` (наше без ответа) → пропуск + INFO-лог; входящее/пусто → шлём.
- **103 — run-цикл + text_provider**: `HeartbeatService(client, storage, *, clock, rng,
  text_provider=None)`. Тик — `process_plans(now=)` (тестируется напрямую с инжектированным
  clock, БЕЗ реального цикла); `run()` — тонкая обёртка раз в минуту (gather, НЕ TaskGroup,
  Ctrl+C). За тик: для каждого enabled-плана если `is_due` и предохранители ок → шаблон
  через `rng` ИЛИ `text_provider` (его ошибка/пусто → фолбэк на шаблон), `send_text`,
  обновить `last_ping_at`/`pings_today`/`day_start`, журнал. Ошибка отправки →
  `logger.exception`, цикл жив. Тесты: серия тиков, провайдер-оверрайд, фолбэк, send-error.
- **104 — CLI + финал**: группа `heartbeat`: `plan PEER (--at HH:MM | --interval H)
  --template … [--jitter/--quiet-start/--quiet-end/--max-per-day]` (`--at` = разовый
  native `send_text(schedule=)`, БЕЗ плана; `--interval` = повторяющийся план в storage;
  взаимоисключающие, хотя бы один обязателен), `list`/`remove PEER`, `run` (паттерн watch,
  Ctrl+C, `make_storage` seam). Тесты CliRunner+стаб. CLAUDE.md/PLAN.md/.env.example.

v1-компромиссы: только CLI (без web/TUI); `text_provider` — обычный Callable (НЕ LLM-импорт,
суфлёр инжектируется снаружи); состояние счётчика/расписания живёт в SQLite. TZ-допущение:
`now` — Unix-таймстамп стенных часов, quiet hours считаются в ЛОКАЛЬНОЙ TZ через
`datetime.fromtimestamp(now)` (`quiet_start`/`quiet_end` — локальные часы 0–23).

## Циклы 105–112 — интероп (#20, сделано)

Связка двух проектов по ролям: **messenger = «руки»** (читает/шлёт через аккаунт),
**factory = «память и поиск»** (индекс переписки + очередь заданий). **Ключевой инвариант:
httpx ТОЛЬКО в `interop/`** — `core/` его не импортирует (packaging-тест стережёт),
`agent/tools.py` тянет `interop` лениво внутри тел инструментов. Новый extra `[interop]`
(+ в `[all]`).

- **105 — `FactoryClient.search_messages`** (`interop/factory_client.py`): async httpx-обёртка,
  `http`/`sleep` инжектируемы (тесты — `httpx.MockTransport` + no-op sleep, респкс не стоит).
  **Авторизация — HTTP Basic с ПУСТЫМ логином + общий пароль** (`httpx.BasicAuth("", password)`);
  это мок-контракт, фабрика подстраивается встречным ишью. 401 → понятный `FactoryError`
  (не голый httpx); транзиентные `httpx.TransportError` → 3 попытки с линейным бэкофом через
  инжектированный sleep; 4xx/5xx (не сеть) → сразу наверх без ретраев.
- **106 — task-методы**: `create_task` (штампует `payload["v"]=1`), `get_task`,
  `claim_next(types)` (404/пусто → None), `complete_task`, `fail_task`; Pydantic `InteropTask`.
- **107 — `Worker.dm_reply`** (`interop/worker.py`): цикл по паттерну `AgentRunner` —
  poll → claim → execute → complete|fail, ошибка одной задачи НЕ убивает цикл
  (`logger.exception` + `fail_task`). `process_once()` — один тестируемый шаг.
  `dm_reply`/`chat_answer`: `{peer, text}` → `send_text` → `{sent: msg_id}`.
- **108 — `fetch_history`/`fetch_dialogs`**: чтение → `model_dump(mode="json")` →
  `{messages|dialogs: [...]}`.
- **109 — prompt-задачи через agent**: `{peer, prompt}` → инжектированный `agent.handle`;
  без агента — `fail_task` с понятным «requires the [agent] extra» (этот путь — единственный
  под `importorskip`, остальные interop-тесты langchain-free).
- **110 — агентские инструменты**: `factory_search`/`factory_create_task` в `make_telegram_tools`
  ТОЛЬКО при `factory_url` (из `TG_FACTORY_URL`/`TG_FACTORY_PASSWORD` на `AgentConfig`); строятся
  через seam `_make_factory_client` (тесты патчат — без httpx).
- **111 — CLI `worker`**: `tg-messenger worker --factory-url URL [--types …] [--interval N]`,
  ленивый импорт interop → `ClickException` с подсказкой `pip install 'tg-messenger[interop]'`,
  Ctrl+C чисто; seam `make_factory_client` + патч `Worker`.
- **112 — финал**: `[interop]` в pyproject (+ `[all]`), `.env.example`/README/CLAUDE.md/PLAN.md;
  accepted v1 risk — полное доверие фабрике (задачи исполняются аккаунтом без авторизации
  источника, кроме общего пароля).

## Циклы 113–118 — отправка медиа (#21, сделано)

Файлы/фото/видео/GIF/голосовые из всех UI + `@`-синтаксис в TUI-композере.

- **113 — core-параметры**: `client.send_media(peer, file_path, *, caption=None,
  voice_note=False, video_note=False, force_document=False)` — флаги напрямую в
  telethon `send_file`. **Валидация пути ДО сети**: несуществующий путь или каталог →
  `ValueError("file not found: …")` без сетевого вызова; история инвалидируется при успехе.
  `conftest.send_file` расширен новыми kwargs.
- **114 — CLI**: `send PEER [TEXT] --file PATH [--caption TEXT] [--voice] [--video-note]
  [--as-file]`. Маппинг `--voice→voice_note`, `--video-note→video_note`,
  `--as-file→force_document`; caption — из `--caption` или позиционного TEXT.
  `--voice/--video-note/--as-file` взаимоисключаемы (≥2 → `ClickException`).
- **115 — web upload**: `POST /dialogs/{id}/media` (multipart `UploadFile` + `caption` Form):
  пустой файл → 400, превышение `TG_WEB_MAX_UPLOAD_MB` (default 50) → 413; иначе временный
  файл (tempfile), `send_media`, удаление файла в `finally`, возврат фрагмента-пузыря.
  Кнопка 📎 в `chat.html` (подпись = текст композера).
- **116 — TUI `@`-парсер**: `parse_media_command(text)` — ЧИСТАЯ функция (без ФС, `shlex`):
  `@a.jpg`→`("a.jpg", None)`; `@"с пробелом.png" подпись`→`("с пробелом.png", "подпись")`;
  `@/path/x.jpg caption here`→`("/path/x.jpg", "caption here")`; текст без `@`→`None`.
- **117 — TUI отправка**: `on_input_submitted` парсит первым; медиа-команда с существующим
  путём → `_send_media` в worker + пузырь; путь не найден → `notify(severity=error)` + лог
  (проверка существования ДО worker/сети); обычный текст → прежний `_send_text` (регресс).
- **118 — финал**: README (медиа-флаги CLI, `@PATH` в TUI, 📎 в web), CLAUDE.md
  (client/web/tui/cli), PLAN.md, `.env.example` (`TG_WEB_MAX_UPLOAD_MB`).

## Циклы 119–122 — юзернеймы (#22, сделано)

Генерация, проверка и установка публичного @username собственного аккаунта (core + CLI, без LLM/UI).

- **119 — `generate_candidates`** (`core/usernames.py`): ЧИСТАЯ функция
  `generate_candidates(base, *, count=20, rng=...)` — `rng` ИНЖЕКТИТСЯ (детерминизм,
  без сети, без глобального random). Нормализация `base` под правила Telegram:
  транслит кириллицы маленьким словарём, прочий не-ASCII отбрасывается, ведущие
  цифры/подчёркивания снимаются (юзернейм начинается с буквы), короткий стем
  допадывается. Вариации: цифровые/годовые суффиксы, подчёркивания, умеренный leet
  (o→0…). Все кандидаты валидны по `is_valid_username` (5–32, `[a-z][a-z0-9_]*`, без
  `__`, без хвостового `_`), без дублей, порядок стабилен, `count` — потолок.
- **120 — check/set/clear в core** (`client.py`): `check_username`/`set_username`/
  `clear_username` через telethon `account.CheckUsernameRequest`/`UpdateUsernameRequest`
  с `run_with_flood_wait_retry`. `UsernameInvalidError`/`UsernameOccupiedError` → `ValueError`
  с понятным текстом (не голый traceback); занят → `check` возвращает `False`.
  `conftest` расширен: `occupied_usernames`/`invalid_usernames`/`set_username_to`.
- **121 — `find_available`** (`core/usernames.py`): `find_available(client, base, *,
  limit=10, count=20, rng=...)` — генерирует `count` кандидатов, проверяет их
  ПОСЛЕДОВАТЕЛЬНО (по одному `check_username` — флуд-дисциплина #8, НЕ параллельный
  шквал), собирает свободные в порядке генерации, останавливается на `limit`.
  Сетевых проверок ≤ `count`.
- **122 — CLI + финал**: группа `username`: `suggest BASE [--limit 10]` (печатает
  найденные доступные), `set NAME` (check→set→подтверждение; занят/invalid → ошибка),
  `clear`. CLAUDE.md (core: `usernames.py`), PLAN.md, pytest+ruff.

## Циклы 123–126 — авторизация web (#24, сделано)

Пароль на весь web-интерфейс: HMAC-cookie сессия, защита SSE, безопасные режимы запуска.
Обратная совместимость — главный инвариант: без `TG_WEB_PASS` middleware НЕ добавляется,
существующие web-тесты (`build_app(client=stub)`) зелёные без правок.

- **123 — middleware-гейт** (`web/app.py`): `build_app(..., web_pass=None)`. Если задан —
  `@app.middleware("http")`: запрос без валидной cookie → браузеру (Accept: text/html)
  302 на `/login`, иначе (JSON/SSE) → 401. Белый список без авторизации: `/login`, `/logout`.
  Без `web_pass` middleware отсутствует (полная регрессия).
- **124 — логин/логаут** (`web/app.py` + `templates/login.html`): `GET /login` — форма;
  `POST /login` — `hmac.compare_digest(pass, web_pass)` (constant-time), верно → Set-Cookie
  с подписанным значением `"{expiry}:{hmac_sha256}"` (ключ `secrets.token_bytes` на процесс,
  на `app.state.cookie_key`; срок +7 дней), redirect `/`; неверно → инжектируемый `_login_delay`
  (тесты монкипатчат на no-op, БЕЗ реального sleep) + `logger.warning` с `request.client.host`
  + 401. `GET /logout` — стирает cookie, redirect `/login`. `_valid_cookie` проверяет подпись
  (`compare_digest`) И срок.
- **125 — режимы запуска** (`cli/main.py serve`): читает `TG_WEB_PASS` из env, прокидывает
  `build_app(web_pass=...)`. НЕ-localhost host без пароля и без `--insecure` → `ClickException`
  ("refusing to serve on <host> without TG_WEB_PASS — set it or pass --insecure"). `--insecure`
  → старт без пароля + `logger.warning`. 127.0.0.1 без пароля — как раньше.
- **126 — финал**: тест что SSE (`/stream/{id}`) без cookie → 401; `.env.example` (`TG_WEB_PASS`
  + комментарий про `--host 0.0.0.0` и reverse-proxy для HTTPS), README (раздел web-auth),
  CLAUDE.md, PLAN.md, pytest+ruff.

## Циклы 127–129 — глобальный лимитер исходящих (#25, сделано)

`core/ratelimit.py` — `TokenBucket`: один потолок на ВСЕ исходящие мутации
(send_text/send_media/forward/send_reaction), закрывает 4 источника отправки разом.
- **127**: burst мгновенно; исчерпание → ожидание ровно до следующего токена (fake
  clock + инжектируемый sleep, контроль длительностей); конкурентные acquire (gather)
  сериализуются честной очередью; rate=0 → no-op.
- **128**: интеграция в client — `send_rate_per_min`; при исчерпании отправка ждёт +
  WARNING; выключенный (дефолт 0) → прежнее поведение (все send-тесты без правок).
- **129**: env `TG_SEND_RATE` в make_client, .env.example, CLAUDE.md/PLAN.md.
acquire() — ОДИН раз до retry-контура (ретрай после FloodWait нового токена не берёт).
Чтения не лимитируются (кэш #8).

## Циклы 130–135 — UI-логин (#26, сделано)

Логин в Telegram (телефон→код→2FA) из Web и TUI, поверх общего автомата.
- **130**: `core/auth.py` — `LoginSession` поверх `LoginFlow`: состояние
  `phone`→`code`→`password`→`done` (`property state`); `submit_phone`/`submit_code`/
  `submit_password`/`resend`. Неверный код/пароль → `LoginError` (понятный текст) БЕЗ
  потери состояния (повтор шага); шаг не по порядку → `RuntimeError`. Тесты — без UI.
  Плюс общий `delivery_hint(CodeDelivery)` — текст «куда ушёл код».
- **131**: web-мастер `/tg-login` (за `web_pass`, если задан): GET форма телефона;
  POST `/tg-login/phone`→фрагмент кода с подсказкой; POST `/tg-login/code`→redirect `/`
  или фрагмент 2FA; POST `/tg-login/password`→redirect; resend. `LoginSession` — один
  на процесс в `app.state`, привязан к lifespan-клиенту; `build_app(login_session=)` —
  сим для тестов.
- **132**: неверный код→фрагмент с ошибкой (НЕ 500), состояние держится; успех→
  `client.save_session()`+redirect; **неавторизованный клиент на `/`,`/dialogs`→redirect
  на `/tg-login`** (старый 401-фрагмент заменён; регресс-тест переписан).
- **133**: TUI — при not authorized вместо exit показывается `LoginScreen` (ModalScreen,
  паттерн `ProfileScreen`); вся сеть через `run_worker`, не `await` в хендлере; успех→
  `dismiss(True)`→обычный `_startup` (диалоги).
- **134**: TUI 2FA-ветка; неверный код→`notify`+очистка поля, экран остаётся; Ctrl+C на
  шаге кода→чистый выход (`return_code==0`).
- **135**: тест «телефон/код НЕ в лог-файле» (по содержимому лога); README (логин из
  web/TUI), CLAUDE.md (auth.py: LoginSession), PLAN.md (этот блок).
