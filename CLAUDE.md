...
- **`storage.py`** — `Storage`: stdlib `sqlite3` behind `asyncio.to_thread`, serialised by one `asyncio.Lock` over a single `check_same_thread=False` connection (concurrent `gather` callers never race or deadlock). WAL + `foreign_keys=ON`. Consumers (#16/#17/#19) `register_migrations([...])`; applied in order inside one transaction, versioned by `PRAGMA user_version` (a failing batch rolls back and the version doesn't advance). The `kv` table (JSON values) is always present and un-versioned; `get_value`/`set_value` cover small odds and ends. `default_db_path(profile)` = `~/.tg_messenger/<profile>.db` (one DB per profile, #11). **The TTL read cache does NOT live here** — it stays in-memory (#8); `client.py` does not depend on storage (the client stays light; storage is a sibling for services, watch.py pattern).
